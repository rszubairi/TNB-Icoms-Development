...
@Component({
  selector: 'app-outage-create',
  standalone: true,
  imports: [FormsModule],
  templateUrl: './outage-create.component.html',
  styleUrl: './outage-create.component.css'
})
export class OutageCreateComponent {
  ...
  sequences = signal<DropdownValue[]>([]);
  restorations = signal<DropdownValue[]>([]);
  ...
  sequenceId = signal<number | null>(null);
  restorationId = signal<number | null>(null);
  ...
  successWarnings = signal<string[]>([]);
  successConflicts = signal<{ outageNumber: string; equipmentName: string | null }[]>([]);
  ...
  selectedJobTypeLabel = computed(() => this.jobTypes().find((j) => j.dropdownValueId === this.jobTypeId())?.valueLabel ?? '');
  requiresProject = computed(() => this.selectedJobTypeLabel() !== '' && this.selectedJobTypeLabel() !== 'Routine Maintenance');
  ...
  constructor() {
    ...
    this.dropdownValueService.listByCategory('Sequence').subscribe({ next: (v) => this.sequences.set(v), error: () => {} });
    this.dropdownValueService.listByCategory('Restoration').subscribe({ next: (v) => this.restorations.set(v), error: () => {} });
    this.dropdownValueService.listByCategory('JobType').subscribe({ next: (v) => this.jobTypes.set(v), error: () => {} });
    this.projectService.list(true).subscribe({ next: (p) => this.projects.set(p), error: () => {} });

    const zone = this.authService.currentUser()?.zoneId;
    if (zone) {
      this.zoneId.set(Number(zone));
    }
  }
  ...
  private buildPayload(submitNow: boolean) {
    if (!this.stationId() || !this.voltageLevelId() || !this.equipmentTypeId() || !this.primaryEquipmentId() || !this.jobTypeId()) {
      return null;
    }
  ...
  saveDraft(): void {
    ...
    if (!payload) {
      this.formError.set('Zone, station, voltage, equipment, job type, and dates are required.');
      return;
    }
    this.submit(payload);
  }
  ...
  submitOutage(): void {
    ...
    if (!payload.description) {
      this.formError.set('Description is required to submit.');
      return;
    }
    if (this.requiresProject() && !payload.projectId) {
      this.formError.set('A project is required for this job type.');
      return;
    }
    this.submit(payload);
  }

  private submit(payload: ReturnType<OutageCreateComponent['buildPayload']>): void {
    if (!payload) return;

    this.formError.set(null);
    this.saving.set(true);

    this.outageService.create(payload).subscribe({
      next: (outage) => {
        this.saving.set(false);
        this.successMessage.set(`Outage ${outage.outageNumber} created (status: ${outage.requestorStatus}).`);
        this.successWarnings.set(outage.warnings);
        this.successConflicts.set(outage.conflictingOutages.map((c) => ({ outageNumber: c.outageNumber, equipmentName: c.equipmentName })));
      },
      error: (err) => {
        this.saving.set(false);
        this.formError.set(err?.error?.error ?? err?.message ?? 'Unable to create outage.');
      }
    });
  }

  createAnother(): void {
    this.router.navigateByUrl('/outages/new').then(() => window.location.reload());
  }

  goToRepository(): void {
    this.router.navigateByUrl('/outages/repository');
  }
}
